...
def _value_parser(value: str) -> Union[str, int, bool]:
    res = value
    if res[-1] == ',':
        res = res[:-1]
    if '"' in res:
    ...
    while _can_continue_parsing(file_handle, terminator):
        ...
        try:
            if isinstance(obj, dict):
                k, v = [_value_parser(segment) for segment in segments]
                obj[k] = v
            else:
                obj.append(_value_parser(segments[0]))
        except Exception as e:
            if isinstance(obj, dict):
                k, v = _value_parser(segments[0]), segments[1]
